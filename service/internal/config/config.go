package config

import "os"

type Config struct {
	AppName         string
	Env             string
	Port            string
	DatabaseURL     string
	CoreAPIBase     string
	AllowedOrigins  string
	UploadsDir      string
	UploadsPublic   string
	UploadsProvider string
}

func getenv(k, def string) string {
	if v := os.Getenv(k); v != "" {
		return v
	}
	return def
}

func Load() Config {
	return Config{
		AppName:         getenv("APP_NAME", "berjis-slides"),
		Env:             getenv("APP_ENV", "development"),
		Port:            getenv("PORT", "8085"),
		DatabaseURL:     getenv("DATABASE_URL", "postgres://postgres:postgres@localhost:5438/berjis_slides?sslmode=disable"),
		CoreAPIBase:     getenv("CORE_API_BASE", "http://localhost:8080"),
		AllowedOrigins:  getenv("ALLOWED_ORIGINS", "https://berjis.tech,https://*.berjis.tech"),
		UploadsDir:      getenv("UPLOADS_DIR", "./docker-data/uploads"),
		UploadsPublic:   getenv("UPLOADS_PUBLIC_BASE", ""),
		UploadsProvider: getenv("UPLOADS_PROVIDER", "local"),
	}
}
