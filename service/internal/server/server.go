...
func New(opts Options) *fiber.App {
    ...
    type slideIn struct { Title *string `json:"title"`; Data json.RawMessage `json:"data"`; Status *string `json:"status"` }
    // Create
    app.Post("/v1/slides", func(c *fiber.Ctx) error {
        if opts.DB == nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        uid, err := getUID(c); if err != nil { return c.Status(401).JSON(fiber.Map{"success": false}) }
        var in slideIn; if err := c.BodyParser(&in); err != nil { return c.Status(400).JSON(fiber.Map{"success": false}) }
        hasTitle := in.Title != nil && strings.TrimSpace(*in.Title) != ""
        hasData := len(in.Data) > 2
        if !hasTitle && !hasData { return c.Status(400).JSON(fiber.Map{"success": false, "message": "empty"}) }
        var s Slide
    ...
    app.Put("/v1/slides/:id", func(c *fiber.Ctx) error {
        if opts.DB == nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        uid, err := getUID(c); if err != nil { return c.Status(401).JSON(fiber.Map{"success": false}) }
        id := c.Params("id"); var in slideIn; if err := c.BodyParser(&in); err != nil { return c.Status(400).JSON(fiber.Map{"success": false}) }
        var s Slide
